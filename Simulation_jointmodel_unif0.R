...
#############################################################
X <- as.matrix(read.csv(list.files(pattern="X_data0.")))
Y <- as.matrix(read.csv(list.files(pattern="Y_data0.")))
simdat.pe00 <- as.data.frame(read.csv(list.files(pattern="sim.pe_data0.")))
#############################################################
...
model { 
  ...
  text <- list.files(pattern="X_data0.")
  num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[2]]))
  write.csv(result_df, paste0("result0.",num,".csv"))
  ...
  #plot(vars[,1])
  #summary(vars)
  pdf(file = paste0("traceplot0.",num,".pdf"),   # The directory you want to save the file in
