...
setwd("/Users/Shared/Windows/UCHealth/RA/Project/EPIC-CF/Analysis_Jiayuan/EPIC_Sim_Results/mixnewJM2_031325")

###########################################################################
# Read csv files
text <- list.files(pattern="mixJM.newresult5.")
num <- as.numeric(unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[3]])))

data_frames <- lapply(num, function(i) {
  file_name <- paste0("mixJM.newresult5.", i, ".csv") 
  read.csv(file_name)
})
...
for(i in 1:I){ 
  ...
  cp1.mean[i] <-mean(data_frames[[i]][8:807,5])
  pi1.mean[i] <-data_frames[[i]][808,5] 
  pi2.mean[i] <-data_frames[[i]][809,5] 
  pi1r.mean[i] <-data_frames[[i]][810,5] 
  pi2r.mean[i] <-data_frames[[i]][811,5] 
  #u1.mean[i] <-mean(data_frames[[i]][812:1611,5])
  u1.tau.inv.mean[i] <-data_frames[[i]][812,5]
  #u2.mean[i] <-mean(data_frames[[i]][1612:2011,5])
  u2.tau.inv.mean[i] <-data_frames[[i]][813,5]
  cp1.mu.mean[i] <-data_frames[[i]][816,5]
  cp1.tau.mean[i] <-data_frames[[i]][817,5]
  
  b10.mean[i] <-data_frames[[i]][819,5] 
  b20.mean[i] <-data_frames[[i]][820,5] 
  b1.mean[i] <-data_frames[[i]][821,5] 
  b2.mean[i] <-data_frames[[i]][822,5] 
  a.mean[i] <-data_frames[[i]][823,5] 
  ga10.mean[i] <-data_frames[[i]][824,5] 
  ga20.mean[i] <-data_frames[[i]][825,5] 
  ga11.mean[i] <-data_frames[[i]][826,5] 
  #w1.mean[i] <-mean(data_frames[[i]][2027:2426,5])
  w1.tau.inv.mean[i] <-data_frames[[i]][829,5]
  #w2.mean[i] <-mean(data_frames[[i]][2427:2826,5])
  w2.tau.inv.mean[i] <-data_frames[[i]][830,5]
}
