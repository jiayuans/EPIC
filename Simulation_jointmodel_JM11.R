...
#############################################################
X <- as.matrix(read.csv(list.files(pattern="X_dataJM11.")))
Y <- as.matrix(read.csv(list.files(pattern="Y_dataJM11.")))
simdat.pe00 <- as.data.frame(read.csv(list.files(pattern="sim.pe_dataJM11.")))
#############################################################
...
model { 
  ...
  ##initial Values
  inits1 <- dump.format(list(c0=-3.8, c=c(0.17,-0.02,0.1,0.06), u.tau=1, u.tau1=11, u.tau2=25, u.tau3=25, cp1=6.9, cp2=14.5,
                             b0=-4.27, b=0.25, a=1.76, w.tau=4.9, ga=0.1,ga1=0.15,ga2=0.2,ga3=0.2,
                             .RNG.name="base::Super-Duper", .RNG.seed=1))
  inits2 <- dump.format(list(c0=-3.81, c=c(0.17,-0.02,0.1,0.06)+0.01, u.tau=1, u.tau1=11, u.tau2=25, u.tau3=25, cp1=7, cp2=14.6,
                             b0=-4.28, b=0.26, a=1.77, w.tau=5, ga=0.11,ga1=0.16,ga2=0.3,ga3=0.3,
  ...
  text <- list.files(pattern="X_dataJM11.")
  num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[2]]))
  write.csv(result_df, paste0("resultJM11.",num,".csv"))
  ...
  #plot(vars[,1])
  #summary(vars)
  pdf(file = paste0("traceplotJM11.",num,".pdf"),   # The directory you want to save the file in
