...
for (r in 2:I){
  ...
  simdat.pe00 <- poisson.d(alpha=1.76,beta=0.25,beta0=-4.27,x=X1,ga=0.11,ga1=0.16,ga2=8,ga3=25,TTei=tt-0.25)
  
  X_df <- as.data.frame(X)
  filename <- paste0("X_dataJM1.", r-2, ".csv")
  write.csv(X_df, file = filename, row.names = FALSE)
  
  Y_df <- as.data.frame(Y)
  filename <- paste0("Y_dataJM1.", r-2, ".csv")
  write.csv(Y_df, file = filename, row.names = FALSE)
  
  simdat.pe_df <- as.data.frame(simdat.pe00)
  filename <- paste0("sim.pe_dataJM1.", r-2, ".csv")
