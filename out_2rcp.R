...
setwd("/Users/Shared/Windows/UCHealth/RA/Project/EPIC-CF/Analysis_Jiayuan/EPIC_Sim_Results/JM2rcpcp_042525")

###########################################################################
# Read csv files
text <- list.files(pattern="result_2rcpcp.")
num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[2]]))

data_frames <- lapply(num, function(i) {
  file_name <- paste0("result_2rcpcp.", i, ".csv") 
  read.csv(file_name)
})
...
cp1mu.mean[i] <-data_frames[[i]][817,5] 
cp1var.mean[i] <-data_frames[[i]][818,5] 
...
Sim.results=cbind(Flag,B1.mean,B2.mean,B3.mean,cp1.mean,cp2.mean,c0.mean,c1.mean,c2.mean,c3.mean,c4.mean,u.tau.inv.mean,
               b0.mean,b1.mean,a.mean,ga.mean,ga1.mean,ga2.mean,w.tau.inv.mean,cp1mu.mean,cp1var.mean,u.mean,v.mean,w.mean)
