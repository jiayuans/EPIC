...
#############################################################
X <- as.matrix(read.csv(list.files(pattern="X_data_1rcprs.")))
Y <- as.matrix(read.csv(list.files(pattern="Y_data_1rcprs.")))
simdat.pe00 <- as.data.frame(read.csv(list.files(pattern="sim.pe_data_1rcprs.")))
#############################################################
...
model { 
  ...
  ### PA model
        Y[i,j] ~ dbin(p2[i,j],1)
        logit(p2[i,j]) <- c0 + (c[1]+u1[i]) * (X[i,j]-cp1[i]) + (c[2]+u2[i]) * (X[i,j]-cp1[i]) * (2*step(X[i,j]-cp1[i])-1) + c[3] * X1[i] + u[i]
        }
        for(j in 1:k.pe[i]){
  ### PE model
       ## Weibull baseline
        ...
        ll.a[i] <- log(L.a[i])
        w[i] ~ dnorm(0,w.tau)
        v[i] <- exp(ga0*u[i]+w[i]+ga*cp1[i]+ga1*u1[i]+ga2*u2[i])
  ...
  u.tau1 ~ dgamma(0.01,0.01)
	u.tau2 ~ dgamma(0.01,0.01)
	cp1.mu ~ dnorm(0,0.01)
  ...
  ##initial Values
  inits1 <- dump.format(list(c0=-3, c=c(0.09,0.01,-0.05), u.tau=0.04, u.tau1=0.04, u.tau2=0.04, cp1.mu=8, cp1.tau=1, 
                             b0=-2, b=0.2, a=1.8, w.tau=0.04, ga0=0.1, ga=-0.05, ga1=-0.1, ga2=1,
                             .RNG.name="base::Super-Duper", .RNG.seed=1))
  inits2 <- dump.format(list(c0=-3.1, c=c(0.09,0.01,-0.05)+0.01, u.tau=0.04, u.tau1=0.04, u.tau2=0.04, cp1.mu=8.1, cp1.tau=1, 
                             b0=-2.1, b=0.21, a=1.8, w.tau=0.04, ga0=0.11, ga=-0.04,ga1=-0.09, ga2=1.1,
  ...
  res <- run.jags(model=modelrancp, burnin=5000, sample=5000, 
                  ...
                  data=data, n.chains=2, method = "parallel",inits=c(inits1,inits2), thin=12)
  
  summary <- summary(res)
  result_df <- as.data.frame(summary)
  text <- list.files(pattern="X_data_1rcprs.")
  num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[2]]))
  write.csv(result_df, paste0("result_1rcprs.",num,".csv"))
  save(res, file=paste0("res_1rcprs.",num,".RData"))
  
  res_jm <- res$mcmc
  vars<-mcmc.list(res_jm[[1]][,c(1:2,403:418)],res_jm[[2]][,c(1:2,403:418)])
  pdf(file = paste0("traceplot_1rcprs.",num,".pdf"),   # The directory you want to save the file in
