...
#############################################################
X <- as.matrix(read.csv(list.files(pattern="X_data_1rcpo.")))
Y <- as.matrix(read.csv(list.files(pattern="Y_data_1rcpo.")))
simdat.pe00 <- as.data.frame(read.csv(list.files(pattern="sim.pe_data_1rcpo.")))
#############################################################
...
model { 
  ...
  ### PE model
       ## Weibull baseline
        ...
        z_u[i]  ~ dnorm(0,1); u[i]    <- sigma_u   * z_u[i]
        z_w[i]  ~ dnorm(0,1); w[i]    <- sigma_w   * z_w[i]
        z_cp[i] ~ dnorm(0,1); cp1[i]  <- cp1.mu + sigma_cp1 * z_cp[i]
        cp1.c[i] <- sigma_cp1 * z_cp[i]   # centered automatically
  ...
  b ~ dnorm(0,0.0001)
	...
	sigma_u   <- 1/sqrt(u.tau)
  sigma_w   <- 1/sqrt(w.tau)
  sigma_cp1 <- 1/sqrt(cp1.tau)
	logv.offset <- -0.5 * ( pow(ga,2)*pow(sigma_u,2)
                      + pow(sigma_w,2)
                      + pow(ga1,2)*pow(sigma_cp1,2) )
}"
  ...
  text <- list.files(pattern="X_data_1rcpo.")
  num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[2]]))
  write.csv(result_df, paste0("result_1rcpo.",num,".csv"))
  save(res, file=paste0("res_1rcpo.",num,".RData"))
  
  res_jm <- res$mcmc
  vars<-mcmc.list(res_jm[[1]][,c(1:2,403:413)],res_jm[[2]][,c(1:2,403:413)])
  pdf(file = paste0("traceplot_1rcpo.",num,".pdf"),   # The directory you want to save the file in
