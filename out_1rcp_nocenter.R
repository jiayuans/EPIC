...
setwd("/Users/Shared/Windows/UCHealth/RA/Project/EPIC-CF/Analysis_Jiayuan/EPIC_Sim_Results/1RCPNC_0917")
setwd("/Volumes/dept/SPH/SPH-BIOS/EJCStudents/ShiJ/EPIC-CF/Simulation/1RCPC_0914") ##### best estimates in folder 1RCP_new
load("/Volumes/dept/SPH/SPH-BIOS/EJCStudents/ShiJ/EPIC-CF/Simulation/1RCP_new/res_1rcp.1.RData")


###########################################################################
# Read csv files
text <- list.files(pattern="result_1rcpnc.")
num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[2]]))

data_frames <- lapply(num, function(i) {
  file_name <- paste0("result_1rcpnc.", i, ".csv") 
  read.csv(file_name)
})
