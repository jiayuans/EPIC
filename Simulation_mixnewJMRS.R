...
#############################################################
X <- as.matrix(read.csv(list.files(pattern="mixJM.X_newdatars.")))
Y <- as.matrix(read.csv(list.files(pattern="mixJM.Y_newdatars.")))
simdat.pe00 <- as.data.frame(read.csv(list.files(pattern="mixJM.rec_newdatars.")))
#############################################################
...
model { 
  ...
  for(i in 1:N){ 
    ...
    for(j in 1:k.pa[i]){
      ...
      logit(p[i,j,1]) <- c10
        + (c[1] + u11[i]) * (X[i,j] - cp1[i])
        + (c[2] + u12[i]) * (X[i,j] - cp1[i]) * (2*step(X[i,j] - cp1[i]) - 1)
        + c[3] * X1[i]
        + u10[i]
    ...
    w1[i] ~ dnorm(0, w.tau1)
    w2[i] ~ dnorm(0, w.tau2)
    v1[i] <- exp(ga10*u10[i] + w1[i] + ga*cp1c[i] + ga11*u11[i] + ga12*u12[i])
    v2[i] <- exp(ga20*u20[i] + w2[i] + ga21*u21[i])
  ...
  u.tau10 ~ dgamma(16,4) # u.tau1 ~ dgamma(0.001,0.001)
  u.tau.inv1 <- 1/u.tau10
  u.tau20 ~ dgamma(16,4) 
  u.tau.inv2 <- 1/u.tau20
  u.tau11 ~ dgamma(16,4)
	u.tau.inv11 <- 1/u.tau11  
	u.tau12 ~ dgamma(16,4)
	u.tau.inv12 <- 1/u.tau12  
	u.tau21 ~ dgamma(16,4)
	u.tau.inv21 <- 1/u.tau21  
...
###initial Values
inits1 <- dump.format(list(c20=-2.6, delta_c=0.7, c=c(0.3,0.3,-0.05), pi=c(0.45,0.55), pi.r=c(0.6,0.4), u.tau10=6.25,u.tau20=6.25,u.tau11=25,u.tau12=25,u.tau21=25, cp1.mu=10, cp1.tau=1,
                           b20_raw=-2, delta_b=2, b=c(0.2,0.3), a1=2.5,a2=0.5, w.tau1=11.1, w.tau2=11.1, ga10=0.8,ga20=0.5,ga=-0.1,ga11=-0.2,ga12=0.3,ga21=0.7,
                           .RNG.name="base::Super-Duper", .RNG.seed=1)) 
inits2 <- dump.format(list(c20=-2.5, delta_c=0.6, c=c(0.3,0.3,-0.05)+0.01, pi=c(0.44,0.56), pi.r=c(0.59,0.41),u.tau10=6.25,u.tau20=6.25,u.tau11=25,u.tau12=25,u.tau21=25, cp1.mu=10.1, cp1.tau=0.9,
                           b20_raw=-1.9, delta_b=2.2, b=c(0.2,0.3)+0.1, a1=2.45,a2=0.45, w.tau1=10, w.tau2=12, ga10=0.9,ga20=0.6,ga=-0.08,ga11=-0.2,ga12=0.4,ga21=0.8,
                           .RNG.name="base::Super-Duper", .RNG.seed=2))

#### Run the model and produce plots
res <- run.jags(model=modelrancp, burnin=10000, sample=5000,  
                monitor=c("B1","B2","c10", "c20","c", "cp1",
                          "pi","pi.r","u.tau.inv1","u.tau.inv2", "u.tau10","u.tau20","u.tau11","u.tau12","u.tau21",
                          "cp1.mu","cp1.tau.inv","cp1.tau",
                          "b10","b20","b", "a1","a2","ga10","ga20","ga","ga11","ga12","ga21",
...
text <- list.files(pattern="mixJM.X_newdatars.")
num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[3]]))
write.csv(result_df, paste0("mixJM.newresultrs.",num,".csv"))
save(res, file=paste0("mixJM.newresrs.",num,".RData"))

res_jm <- res$mcmc
vars<-mcmc.list(res_jm[[1]][,c(1:16)],res_jm[[2]][,c(1:16)])
pdf(file = paste0("mixJM.newtraceplotrs.",num,".pdf"),   # The directory you want to save the file in
