...
for (r in 2:I){
  ...
  for (i in 1:N){
    for (j in 1:k.pa[i]){
      ...
      p1[i,j]=exp(c10+(c1+b_11i[i])*(X[i,j]-cp_i[i])+(c2+b_12i[i])*(X[i,j]-cp_i[i])*I[i,j]+b_10i[i])/(1+exp(c10+(c1+b_11i[i])*(X[i,j]-cp_i[i])+(c2+b_12i[i])*(X[i,j]-cp_i[i])*I[i,j]+b_10i[i]))
      p2[i,j]=exp(c20+(c1-c2+b_21i[i])*X[i,j]+b_20i[i])/(1+exp(c20+(c1-c2+b_21i[i])*X[i,j]+b_20i[i]))
  ...
  filename <- paste0("mixLM.X_data1.", r-2, ".csv")
  write.csv(X_df, file = filename, row.names = FALSE)
  
  Y_df <- as.data.frame(Y)
  filename <- paste0("mixLM.Y_data1.", r-2, ".csv")
