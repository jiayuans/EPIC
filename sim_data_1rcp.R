...
for (r in 2:I){
  ...
  for (i in 1:N){
    for (j in 1:k.pa[i]){
      ...
      p2[i,j]=exp(c0+c1*(X[i,j]-cp_i[i])+c2*(X[i,j]-cp_i[i])*I[i,j]+c3*X1[i]+b_0i[i])/(1+exp(c0+c1*(X[i,j]-cp_i[i])+c2*(X[i,j]-cp_i[i])*I[i,j]+c3*X1[i]+b_0i[i]))
      Y[i,j]=rbinom(Verror, 1, p2[i,j])
    }
  }
  
  simdat.pe00 <- poisson.d(alpha=1.8,beta=0.2,beta0=-2,x=X1,ga0=0.3,ga=-0.05,TTei=tt-0.25) #ga0=0.3
