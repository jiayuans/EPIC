...
model { 
  ...
  ### PA model
        Y[i,j] ~ dbin(p2[i,j],1)
        logit(p2[i,j]) <- c0 + c[1] * (X[i,j]-cp1) + c[2] * (X[i,j]-cp1) * (step(X[i,j]-cp1)) + c[3] * (X[i,j]-cp2) * (step(X[i,j]-cp2)) + c[4] * X1[i] + u[i]
        }
        for(j in 1:k.pe[i]){
  ### PE model
       ## Weibull baseline
  ...
  ##initial Values
  inits1 <- dump.format(list(c0=-3.7, c=c(-0.15,0.3,0.15,0.1), u.tau=1, cp1=4.5, cp2=14.5,
                             b0=-4.5, b=0.25, a=1.8, w.tau=1, ga=0.25,
                             .RNG.name="base::Super-Duper", .RNG.seed=1))
  inits2 <- dump.format(list(c0=-3.71, c=c(-0.15,0.3,0.15,0.1)+0.01, u.tau=1, cp1=4.6, cp2=14.6,
