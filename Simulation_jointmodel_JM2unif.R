...
long.time <- read.csv("long.data_unif.csv")
...
#############################################################
X <- as.matrix(read.csv(list.files(pattern="X_dataJM2.")))
Y <- as.matrix(read.csv(list.files(pattern="Y_dataJM2.")))
simdat.pe00 <- as.data.frame(read.csv(list.files(pattern="sim.pe_dataJM2.")))
#############################################################
...
model { 
  ...
  ##initial Values
  inits1 <- dump.format(list(c0=-4.5, c=c(0.1,0.13,0.08,0.1), u.tau=1, cp1=4.5, cp2=14.5,
                             b0=-4.3, b=0.25, a=1.8, w.tau=1, ga=0.25,
                             .RNG.name="base::Super-Duper", .RNG.seed=1))
  inits2 <- dump.format(list(c0=-4.51, c=c(0.1,0.13,0.08,0.1)+0.01, u.tau=1, cp1=4.6, cp2=14.6,
                             b0=-4.31, b=0.26, a=1.81, w.tau=1, ga=0.26,
  ...
  res <- run.jags(model=modelrancp, burnin=10000, sample=3000, 
                  ...
                  data=data, n.chains=2, inits=c(inits1,inits2), thin=4, module='dic')
  
  summary <- summary(res)
  result_df <- as.data.frame(summary)
  text <- list.files(pattern="X_dataJM2.")
  num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[2]]))
  write.csv(result_df, paste0("resultJM2.",num,".csv"))
  ...
  #plot(vars[,1])
  #summary(vars)
  pdf(file = paste0("traceplotJM2.",num,".pdf"),   # The directory you want to save the file in
