...
model { 
  ...
  cp1.mu ~ dnorm(0,0.01)
	...
	a ~ dgamma(0.01,0.01)
  b10 ~ dnorm(0,0.25)	# dnorm(0,0.0001)	
  b20 ~ dnorm(0,0.25)	# dnorm(0,0.0001)	
  for (p in 1:2){
	     b[p] ~ dnorm(0,0.25)	# dnorm(0,0.0001)		 
  }
	ga10 ~ dnorm(0,0.0001)
	ga20 ~ dnorm(0,0.0001)
	ga11 ~ dnorm(0,0.0001)
	w.tau1 ~ dgamma(2,2) # dgamma(0.001,0.001)
	w.tau.inv1 <- 1/w.tau1  ## variance 
	w.tau2 ~ dgamma(2,2) # dgamma(0.001,0.001)
...
pdf(file = paste0("mixJM.newtraceplot1.",num,".pdf"),   # The directory you want to save the file in
