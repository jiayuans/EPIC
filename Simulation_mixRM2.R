...
model { 
  ...
  ### PE model
       ## Weibull baseline
        ...
        #zeros[i] ~ dpois(phi[i])
        z.r[i] ~ dcat(pi.r[1:2]) 
        logL.sel[i] <- equals(z.r[i],1) * logL.e1[i] + equals(z.r[i],2) * logL.e2[i]
        phi[i] <-  max(-logL.sel[i] + 1000, 0)
        zeros[i] ~ dpois(phi[i])
  }
  log_lik0.e <- sum(logL.sel[]) 
  dev.e <- -2*log_lik0.e
...
res <- run.jags(model=modelrancp, burnin=10000, sample=3000,  
                monitor=c("b10","b20","b", "a1","a2","ph1","ph2","pi.r","z.r","v1","v2"), 
