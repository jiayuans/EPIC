...
long.time <- read.csv("long.data_new500.csv")
...
#############################################################
X <- as.matrix(read.csv(list.files(pattern="X_dataJM1.")))
Y <- as.matrix(read.csv(list.files(pattern="Y_dataJM1.")))
simdat.pe00 <- as.data.frame(read.csv(list.files(pattern="sim.pe_dataJM1.")))
#############################################################
...
model { 
  ...
  ##initial Values
  inits1 <- dump.format(list(c0=-3.8, c=c(0.17,-0.02,0.1,0.06), u.tau=0.5, u.tau1=12.5, u.tau2=100, u.tau3=625, cp1=6.9, cp2=14.5,
                             b0=-4.27, b=0.25, a=1.76, w.tau=5, ga=0.11,ga1=0.16,ga2=8,ga3=25,
                             .RNG.name="base::Super-Duper", .RNG.seed=1))
  inits2 <- dump.format(list(c0=-3.81, c=c(0.17,-0.02,0.1,0.06)+0.01, u.tau=0.5, u.tau1=12.5, u.tau2=100, u.tau3=625, cp1=7, cp2=14.6,
                             b0=-4.28, b=0.26, a=1.77, w.tau=5.1, ga=0.12,ga1=0.17,ga2=8.1,ga3=25.1,
  ...
  text <- list.files(pattern="X_dataJM1.")
  num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[2]]))
  write.csv(result_df, paste0("resultJM1.",num,".csv"))
  ...
  #plot(vars[,1])
  #summary(vars)
  pdf(file = paste0("traceplotJM1.",num,".pdf"),   # The directory you want to save the file in
