...
#############################################################
X <- as.matrix(read.csv(list.files(pattern="X_data2.")))
Y <- as.matrix(read.csv(list.files(pattern="Y_data2.")))
simdat.pe00 <- as.data.frame(read.csv(list.files(pattern="sim.pe_data2.")))
#############################################################
...
model { 
  ...
  ###initial Values
  inits1 <- dump.format(list(c0=-4.5, c=c(0.1,0.2,0.1,0.1), u.tau=1, cp1=4.5, cp2.temp=10,
                             b0=-4.5, b=0.25, a=1.8, w.tau=1, ga=0.25,
                             .RNG.name="base::Super-Duper", .RNG.seed=1))
  inits2 <- dump.format(list(c0=-4.51, c=c(0.1,0.2,0.1,0.1)+0.01, u.tau=1, cp1=4.6, cp2.temp=10,
  ...
  text <- list.files(pattern="X_data2.")
  num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[2]]))
  write.csv(result_df, paste0("result2.",num,".csv"))
  ...
  #plot(vars[,1])
  #summary(vars)
  pdf(file = paste0("traceplot2.",num,".pdf"),   # The directory you want to save the file in
