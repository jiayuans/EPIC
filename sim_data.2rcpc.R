...
poisson.d <- function(alpha,beta,beta0,x,ga,ga1,ga2,TTei){
  le <- length(x)
  c_0i <- rnorm(le,0,0.2) 
  vi <- exp(ga*b_0i+c_0i+ga1*cp1c+ga2*cp2c)
...
for (r in 2:I){
  ...
  simdat.pe00 <- poisson.d(alpha=1.8,beta=0.2,beta0=-2,x=X1,ga=0.2,ga1=-0.05,ga2=-0.02,TTei=tt-0.25) #ga2=-0.01
  
  X_df <- as.data.frame(X)
  filename <- paste0("X_data_2rcpc.", r-2, ".csv")
  write.csv(X_df, file = filename, row.names = FALSE)
  
  Y_df <- as.data.frame(Y)
  filename <- paste0("Y_data_2rcpc.", r-2, ".csv")
  write.csv(Y_df, file = filename, row.names = FALSE)
  
  simdat.pe_df <- as.data.frame(simdat.pe00)
  filename <- paste0("sim.pe_data_2rcpc.", r-2, ".csv")
