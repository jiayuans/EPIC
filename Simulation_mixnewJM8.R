...
#############################################################
X <- as.matrix(read.csv(list.files(pattern="mixJM.X_newdata4.")))
Y <- as.matrix(read.csv(list.files(pattern="mixJM.Y_newdata4.")))
simdat.pe00 <- as.data.frame(read.csv(list.files(pattern="mixJM.rec_newdata4.")))
#############################################################
...
text <- list.files(pattern="mixJM.X_newdata4.")
num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[3]]))
write.csv(result_df, paste0("mixJM.newresult8.",num,".csv"))
#save(res, file=paste0("mixJM.newres8.",num,".RData"))

res_jm <- res$mcmc
#vars<-mcmc.list(res_jm[[1]][,c(1:16)],res_jm[[2]][,c(1:16)])
#pdf(file = paste0("mixJM.newtraceplot8.",num,".pdf"),   # The directory you want to save the file in
