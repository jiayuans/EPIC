...
model { 
  ...
  ### PA model
        Y[i,j] ~ dbin(p2[i,j],1)
        logit(p2[i,j]) <- c0 + c[1] * X[i,j] + c[2] * X1[i] + u[i]
        }
        for(j in 1:k.pe[i]){
  ### PE model
       ## Weibull baseline
  ...
  write.csv(result_df, paste0("result2_jm4.",num,".csv"))
  ...
  #plot(vars[,1])
  #summary(vars)
  pdf(file = paste0("traceplot2_jm4.",num,".pdf"),   # The directory you want to save the file in
