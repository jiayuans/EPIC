...
for (r in 2:I){
  ...
  simdat.pe00 <- poisson.d(alpha=1.8,beta=0.2,beta0=-1,x=X1,ga0=0.3,ga=-0.05,TTei=tt-0.25)
  
  X_df <- as.data.frame(X)
  filename <- paste0("X_data_1rcp1.", r-2, ".csv")
  write.csv(X_df, file = filename, row.names = FALSE)
  
  Y_df <- as.data.frame(Y)
  filename <- paste0("Y_data_1rcp1.", r-2, ".csv")
  write.csv(Y_df, file = filename, row.names = FALSE)
  
  simdat.pe_df <- as.data.frame(simdat.pe00)
  filename <- paste0("sim.pe_data_1rcp1.", r-2, ".csv")
