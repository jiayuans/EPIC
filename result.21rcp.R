...
###########################################################################
setwd("/Users/Shared/Windows/UCHealth/RA/Project/EPIC-CF/Analysis_Jiayuan/EPIC_Sim_Results/JM21rcp_052825_ok")
...
# DIC and WAIC
###########################################################################
files <- list.files(pattern = "^dicwaic_21rcp\\.[0-9]+\\.csv$")

file_id <- as.numeric(sub("^dicwaic_21rcp\\.([0-9]+)\\.csv$", "\\1", files))
files <- files[order(file_id)]
